...
{name:'贵州', cityList:[           
{name:'贵阳市', areaList:['市辖区','南明区','云岩区','观山湖区','花溪区','乌当区','白云区','小河区','开阳县','息烽县','修文县','清镇市']},          
{name:'六盘水市', areaList:['钟山区','六枝特区','水城县','盘　县']},          
{name:'遵义市', areaList:['市辖区','红花岗区','汇川区','新浦新区','遵义县','桐梓县','绥阳县','正安县','道真仡佬族苗族自治县','务川仡佬族苗族自治县','凤冈县','湄潭县','余庆县','习水县','赤水市','仁怀市']},           
...
// 更换省份，显示城市
function showPro(provinceName){
	cityTag.options.length=1;
	var pos=provinceName.value;
	for (var  j= 0; j < provinceList[pos].cityList.length; j++) {
		var cityName=provinceList[pos].cityList[j].name;
		cityTag.add(new Option(cityName,j));
	}
	// console.log(cityArray);
	cityTag[1].selected=true;
	showCity(cityTag);
}

// 更换城市，显示区县
function showCity(cityName){
	areaTag.options.length=1;
	var cityName=cityName.options[cityName.selectedIndex].text;
	// alert(cityName);
	for (var i = 0; i < provinceList.length; i++) {
		for (var j = 0; j < provinceList[i].cityList.length; j++) {
			if (provinceList[i].cityList[j].name==cityName){
				for (var z = 0; z < provinceList[i].cityList[j].areaList.length; z++) {
					areaTag.add(new Option(provinceList[i].cityList[j].areaList[z],z));
				}
				
			}
		}
	}
	areaTag[1].selected=true;
	// alert(cityPos);
}